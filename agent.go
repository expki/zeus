...
func (c *chat) GenerateWithTools(ctx context.Context, userMessage string, opts ...GenerateOption) iter.Seq2[AgentEvent, error] {
	return func(yield func(AgentEvent, error) bool) {
		...
		for iteration := 0; iteration < maxIterations; iteration++ {
			...
			if len(toolCalls) == 0 {
				// Strip thinking tags from final response to avoid template issues
				// on subsequent queries (Qwen3 template crashes on </think> in content)
				strippedContent := stripThinkingTags(content)
				for i := len(c.messages) - 1; i >= 0; i-- {
					if c.messages[i].Role == RoleAssistant {
						c.messages[i].Content = strippedContent
						break
					}
				}
			...
			// Update the assistant message with cleaned content and tool calls
			// Must be done before adding tool results since assistant message is currently last
			// Note: Qwen3 templates have issues when assistant messages contain </think> tags
			// and also have tool_calls. The template tries to call .split() on content which
			// fails if content becomes null during processing. To avoid this, we strip thinking
			// tags from the content when there are tool calls and ensure content is never empty.
			assistantContent := content
			if len(toolCalls) > 0 {
				// Strip thinking tags for assistant messages with tool calls
				assistantContent = stripThinkingTags(content)
				if strings.TrimSpace(assistantContent) == "" {
					// Provide meaningful content that won't be null after template processing
					assistantContent = "Using tool."
				}
			}
			for i := len(c.messages) - 1; i >= 0; i-- {
				if c.messages[i].Role == RoleAssistant {
					c.messages[i].Content = assistantContent
					c.messages[i].ToolCalls = toolCalls
					break
				}
			}
			...
			for _, call := range toolCalls {
				...
				// Add tool result to conversation with proper metadata
...
// shouldUseLazyGrammar determines if lazy grammar should be applied.
func shouldUseLazyGrammar(params *ChatParams) bool {
	if params == nil || params.Grammar == "" {
		return false
	}
	if !params.GrammarLazy || len(params.GrammarTriggers) == 0 {
		return false
	}
	// Validate at least one trigger is valid
	for _, t := range params.GrammarTriggers {
		if t.Value != "" || t.Type == TriggerTypeToken {
			return true
		}
	}
	return false
...
func (c *chat) generateWithToolPrompt(ctx context.Context, opts ...GenerateOption) (string, error) {
	...
	// allowing models to output free-form text before tool calls.
	if shouldUseLazyGrammar(chatParams) {
		opts = append(opts, withLazyGrammar(chatParams.Grammar, chatParams.GrammarTriggers))
	}
	// Note: If grammar can't be used lazily, we skip it to avoid crashes.
	// Tool calls are still parsed from output via parseToolCallsNative().
...
// stripThinkingTags removes <think>...</think> blocks from content.
// This is needed because some templates (e.g., Qwen3) have issues when
// assistant messages contain thinking tags alongside tool calls.
func stripThinkingTags(content string) string {
	// First try to extract content after </think> if present
