...
func (c *chat) GenerateWithTools(ctx context.Context, userMessage string, opts ...GenerateOption) iter.Seq2[AgentEvent, error] {
	return func(yield func(AgentEvent, error) bool) {
		...
		for iteration := 0; iteration < maxIterations; iteration++ {
			...
			// Note: Some templates (e.g., Qwen3) strip <think></think> tags and then try to
			// call .split() on the remaining content. If content is empty/null after stripping,
			// the template crashes. We ensure content has meaningful text.
			assistantContent := content
			// Check if content is empty or only contains thinking tags
			stripped := strings.TrimSpace(content)
			stripped = strings.ReplaceAll(stripped, "<think>", "")
			stripped = strings.ReplaceAll(stripped, "</think>", "")
			stripped = strings.TrimSpace(stripped)
			if stripped == "" && len(toolCalls) > 0 {
...
func (c *chat) generateWithToolPrompt(ctx context.Context, opts ...GenerateOption) (string, error) {
	...
	// Grammar for tool calling is currently disabled.
	// The grammar generated by llama.cpp is very strict (JSON-only) and crashes
	// when models output any free-form text (markdown, HTML, explanations) before
	// or around tool calls. Most models do this regardless of their format.
	//
	// llama.cpp provides GrammarLazy and GrammarTriggers for deferred grammar
	// application, but implementing this properly requires monitoring generated
	// text for trigger patterns and dynamically applying grammar mid-generation.
	//
	// TODO: Implement lazy grammar triggering for formats that support it.
	// For now, tool calling works without grammar - we just lose the benefit
	// of constrained generation. Tool calls are still parsed from output.
